...
function client.tick(delta)
    ...
    if InputDown(CfgGetValue("KEYBIND_ADJUST_INACCURACY")) then
        ...
        if InputValue("mousewheel") ~= 0 then
            local value = 0.5
            if STATES.shell_inaccuracy >= 15 then
                value = 1
            end
            if STATES.shell_inaccuracy >= 50 then
                value = 2.5
            end
            if STATES.shell_inaccuracy >= 100 then
                value = 5
            end
    ...
    elseif InputDown(CfgGetValue("KEYBIND_ADJUST_DELAY")) and STATES.quicksalvo.enabled then
        ...
        if InputValue("mousewheel") ~= 0 then
            local value = 0.1
            if G_QUICK_SALVO_DELAY >= 4.99 then
                value = 0.5
            end
            if G_QUICK_SALVO_DELAY >= 14.99 then
                value = 1
            end
            if G_QUICK_SALVO_DELAY >= 30 then
                value = 2
            end
            if G_QUICK_SALVO_DELAY >= 60 then
                value = 5
            end
    ...
    elseif InputDown(KEYBINDS["KEYBIND_ADJUST_ATTACK"]) then -- User change heading and angle of attack event
        ...
        if InputValue("mousedx") ~= 0 then
            local offset = 0.5
            if STATES.input_attack_invert then
                offset = -offset
            end
    ...
    UI_HELPERS.shell_telemetry.combined_transform, UI_HELPERS.shell_telemetry.arrow_pitch_pos, UI_HELPERS.shell_telemetry.arrow_heading_pos =
        FdDrawShellImpactGizmo({
            aim_pos,
...
function client.draw()
    ...
    FdUiContainer(function()
        ...
        FdUiContainer(function()
            ...
            KeybindHint(
                { "KEYBIND_ADJUST_INACCURACY", "scroll" },
